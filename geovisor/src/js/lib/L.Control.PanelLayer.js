    ...
    L.Control.PanelLayers = L.Control.Layers.extend({
        ...
        _createItem: function (obj) {
            ...
            title.innerHTML = `&nbsp${obj.name}&nbsp` || '';
        ...
        _createGroup: function (groupdata, isCollapsed) {
            ...
            if (this.options.collapsibleGroups) {
                ...
                L.DomEvent.on(grouplabel, 'click', function () {
                    if (L.DomUtil.hasClass(groupdiv, 'expan')) {
                        L.DomUtil.removeClass(groupdiv, 'expan');
                        groupexp.innerHTML = ' + ';
                    } else {
                        L.DomUtil.addClass(groupdiv, 'expan');
                        groupexp.innerHTML = ' - ';
                    }
                    self._updateHeight();
                });
        ...
        _initLayout: function () {
            var icono;
            var titleContainer = L.DomUtil.create('div', 'titleContainer');
            var bodyContainer = L.DomUtil.create('div', 'hidden');
            var container = this._container = L.DomUtil.create('div', this.className);
    
            if(this.options.compact)
                icono = L.DomUtil.create('i','fas fa-arrow-circle-down')

                L.DomUtil.addClass(container, 'compact');

                titleContainer.innerHTML = `<p align="center">${icono.outerHTML}  <b>Control de capas</b></p>`
            ...
            if (this.options.collapsed) {
                titleContainer.addEventListener('click', function(e) {
                    ...
                    container.classList.toggle('expan')
                    var elem = container.querySelector('i')

                    if(container.classList.contains('expan')){  
                        bodyContainer.classList.remove('hidden')     
                        elem.classList.remove('fa-arrow-circle-down')
                        elem.classList.add('fa-arrow-circle-up')
                    }
                    else {
            ...
            bodyContainer.appendChild(this._form);
        },
        ...
        _expand: function () {
            L.DomUtil.addClass(this._container, 'expan');
        },
    
        _collapse: function () {
            this._container.className = this._container.className.replace('expan', '');
        },
